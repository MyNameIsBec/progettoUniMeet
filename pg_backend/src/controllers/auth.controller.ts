...
export async function verifica2FA(req: Request, res: Response) {
  try {
    const { tempToken, codice } = req.body;
    const result = await authService.verifica2FA(tempToken, codice);
    return res.status(200).json(result);
  } catch (err) {
    if (err instanceof Error && (err.message === 'Codice non valido o scaduto' || err.message === 'Token non valido')) {
      return res.status(401).json({ error: err.message });
    }
    return res.status(500).json({ error: 'Internal server error' });
  }
...
export async function abilita2FA(req: Request, res: Response) {
  try {
    const { id, ruolo } = req.user!;
    const result = await authService.abilita2FA(id, ruolo);
    return res.status(200).json(result);
  } catch (err) {
    if (err instanceof Error && err.message === '2FA già abilitata') {
      return res.status(409).json({ error: err.message });
    }
    if (err instanceof Error && err.message === 'Utente non trovato') {
      return res.status(404).json({ error: err.message });
    }
    return res.status(500).json({ error: 'Internal server error' });
  }
...
export async function confermaAbilita2FA(req: Request, res: Response) {
  try {
    const { id, ruolo } = req.user!;
    const { codice } = req.body;
    const result = await authService.confermaAbilita2FA(id, ruolo, codice);
    return res.status(200).json(result);
  } catch (err) {
    if (err instanceof Error && (err.message === 'Codice non valido o scaduto' || err.message === '2FA già abilitata')) {
      return res.status(401).json({ error: err.message });
    }
    return res.status(500).json({ error: 'Internal server error' });
  }
...
export async function disabilita2FA(req: Request, res: Response) {
  try {
    const { id, ruolo } = req.user!;
    const { password } = req.body;
    const result = await authService.disabilita2FA(id, ruolo, password);
    return res.status(200).json(result);
  } catch (err) {
    if (err instanceof Error && err.message === 'La 2FA è obbligatoria per gli amministratori') {
      return res.status(403).json({ error: err.message });
    }
    if (err instanceof Error && err.message === 'Password errata') {
      return res.status(401).json({ error: err.message });
    }
    return res.status(500).json({ error: 'Internal server error' });
  }
...
export async function getStato2FA(req: Request, res: Response) {
  try {
    const { id, ruolo } = req.user!;
    const result = await authService.getStato2FA(id, ruolo);
    return res.status(200).json(result);
  } catch (err) {
    if (err instanceof Error && err.message === 'Utente non trovato') {
      return res.status(404).json({ error: err.message });
